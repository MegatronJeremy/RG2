...
\begin{description}
  ...
  \item[high-frequency energy] the share of a buffer's signal in fine detail rather than broad
    gradients, used here to quantify how much a filter blurred an image
  ...
  \item[lag] how far a moving frame trails its own converged still image at the same camera pose, a
    measure of temporal smearing rather than of accuracy
  ...
  \item[tFLIP] temporal FLIP: how much a technique changes between adjacent frames in excess of how
    much the reference changes over the same interval, so a stable result scores low
  ...
  \item[Vulkan] the low-level cross-vendor graphics and compute API this engine is built on, in
    which ray tracing is an optional extension rather than a core feature
  \item[wave / warp] a group of threads executing one instruction in lockstep, called a wave on AMD
    and a warp on NVIDIA. A warp is always 32 threads; RDNA selects 32 or 64 per shader, and the
    driver reports 64 for this engine's compute shaders
  \end{description}
