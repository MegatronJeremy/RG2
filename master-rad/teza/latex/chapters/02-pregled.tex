...
evaluated in Chapter~\ref{ch:analiza} sit on opposite sides of that line (Figure~\ref{fig:traversal}):
NVIDIA's RT cores walk the hierarchy in hardware \cite{nvidia_turing}, while on AMD's RDNA
architectures the traversal loop is driven by shader instructions that issue hardware intersection
tests \cite{amd_rdna_isa}, so traversal competes with shading for the same execution resources. This
is one structural reason to \emph{expect} the same shader to cost differently on the two vendors. It
is not an attribution: Section~\ref{sec:cena} measures that the costs differ, and this evaluation does
not isolate how much of any measured gap comes from this mechanism rather than from clocks, cache, or
driver.
...
\label{fig:traversal}
\end{figure} Vulkan exposes a two-level structure. A bottom-level
