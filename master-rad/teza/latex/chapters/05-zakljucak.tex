...
GPU cost with an A/B benchmark and measures reconstruction quality against a supersampled reference.
...
measurements are taken on a single GPU and a single scene class, so the absolute numbers do not
...
the forward pass. Finally, a second rendering backend (DirectX 12) and measurements across vendors and
generations would generalize the performance story beyond a single RDNA3 GPU.
