...
On the cross-vendor pair reflections are the single most expensive effect (RX 9070 XT 2.87 ms, RTX
5070 2.44 ms), ahead of shadows, global illumination, and ambient occlusion, for a measured real-time
total of 9.97 ms (9070 XT) and 7.24 ms (5070) over a baseline under 2.3 ms. Enabling all four effects
lifts reconstruction quality from FLIP 0.435 / PSNR 15.2 dB / SSIM 0.38 (raster) to FLIP 0.087 /
PSNR 30.7 dB / SSIM 0.87 against the path-traced reference (RX 9070 XT; Table~\ref{tab:quality}).

Two of the sweeps sharpen that picture. The ray-count knee sits at one ray per pixel: going from one
to eight rays costs 78\% more GPU time for a 0.20 dB PSNR gain, because the denoiser and temporal
...
frame-to-frame RMS on a static view from 0.3229 to 0.1949, a 40\% reduction in flicker, for 0.33 ms
