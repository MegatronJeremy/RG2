...
\addlinespace[2pt]
Shading & \texttt{Forward} (\texttt{DefaultLit}): early-Z prepass, sky, inline shadow rays, and the combine with GI/AO/reflections \\
\addlinespace[2pt]
...
Directions are drawn from spatiotemporal blue noise, two decorrelated dimensions per light, spreading
...
\paragraph{The stochastic alternative.} Tracing one ray per light per pixel is exact but scales
...
the variance edge-stop is optional. \texttt{GIDenoise.comp} keeps its outer \`a-trous loop rolled rather
