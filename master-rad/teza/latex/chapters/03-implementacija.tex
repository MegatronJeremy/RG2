...
\caption[Hard versus soft ray-traced shadows]{Hard (left) versus soft (right) ray-traced shadows on the sunlit floor strip: the only
difference is \texttt{render.shadow.soft}. The sun's angular diameter is raised to $3^\circ$, about
six times the physical $0.53^\circ$, since at the true value the penumbra is narrower than what
print resolves.}
\label{fig:shadow-hs}
\end{figure}
