...
run instead of leaving them split by the raster upsamples that happen to sit between them. The
intent is to pay one synchronisation boundary for the run instead of one per chain, although that
benefit is not measured here and the flag stays off, so the reordering is a capability of the graph
rather than a property of the numbers in Chapter~\ref{ch:analiza}.
