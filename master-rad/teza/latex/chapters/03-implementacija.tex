...
inline inside the forward pass. Each grey chain has the same four-stage shape, which is what lets one
denoiser serve all of them. Table~\ref{tab:framegraph} gives the same thing pass by pass.}
\label{fig:overview}
\end{figure}
...
\end{tikzpicture}
\caption{Soft shadows: from point $P$ rays are sampled uniformly by solid angle inside the cone the
...
\caption{Hard (left) versus soft (right) ray-traced shadows on the sunlit floor strip: the only
...
\caption{The stochastic aggregate visibility ratio before (left) and after (right) temporal
...
\begin{tikzpicture}[>={Stealth[]}]
  ...
  \fill (0,0) circle (1.7pt) node[below=3pt] {$P$};
\end{tikzpicture}
\caption{Hemisphere sampling for AO: from point $P$ several short rays are cast over the hemisphere
...
\begin{tikzpicture}[>={Stealth[]}]
  ...
  \fill (0,0) circle (1.7pt) node[below=3pt] {$P$};
\end{tikzpicture}
\caption{Reflections: the reflected direction $R$ of the view vector $V$ about the normal $N$. A
...
\caption{Without reflections (left) versus ray-traced reflections (right), at a near-grazing view
...
\caption{Reflected radiance before compositing (\texttt{render.debugview} 3): a sharp mirror ray,
...
\begin{tikzpicture}[>={Stealth[]}]
  ...
  \fill (0,0) circle (1.7pt) node[below=3pt] {$P$};
\end{tikzpicture}
\caption{Global illumination: cosine-weighted hemisphere sampling around $N$ from point $P$. Every
...
\caption{The scene without (top) and with (bottom) global illumination: one diffuse indirect bounce
...
\end{tikzpicture}
\caption{SVGF denoiser: temporal accumulation with variance estimation, then a variance-guided
\`a-trous spatial filter, shared between GI, AO and reflections.}
\label{fig:svgf}
\end{figure}
...
\caption{The motion vector buffer (\texttt{render.debugview} 1) during a deterministic camera orbit.
...
\caption{The converged reference frame from the built-in path tracer, against which every real-time
