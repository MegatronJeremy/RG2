...
\texttt{render.shadows.mode} 1 versus 2. The raster map has finite resolution and a depth bias, so
contact shadows bleed and edges are softer than the geometry dictates, while the traced version
resolves them against the real geometry. The captures are deterministic (two consecutive captures of
the same configuration are bit-identical), so every difference between the panels is real.}
