...
The graph can also schedule from those edges rather than follow them, grouping the ray-traced chains
into one contiguous compute run so the block could move to an asynchronous queue. Both halves were
built and both measured net-negative: reordering alone moves whole-frame time 0.2\%, inside the
run-to-run spread, and the asynchronous queue costs $+0.675$ ms, $+4.1\%$, because the fork, the join
and the queue-family ownership transfers exceed the 1.1\% of raster work available to overlap.
Grouping compute work is standard advice that this frame has nothing to pay for itself with, so both
flags stay off and the schedule is a capability of the graph rather than a property of any number in
Chapter~\ref{ch:analiza}. The DAG behind it carries one caveat worth stating, since it would bound
any future use: only textures are tracked, so a pass touching an acceleration structure, a bindless
table or a buffer has a real ordering requirement that no edge records. The
\texttt{render.graph.dumpdeps} CVar reports which passes look unconstrained, which keeps that gap
...
ray straight at the light, while the soft path casts \texttt{SHADOW_RAY_COUNT = 2} rays sampling,
uniformly by solid angle, the cone the source subtends at the shaded point (Figure~\ref{fig:shadow}),
averaging the hits into a penumbra factor in $[0,1]$. The half-angle follows the source: half the
angular diameter \texttt{render.shadow.sun\_angle\_deg} for the sun, and $\arcsin(r/d)$ for a local
light of radius \texttt{render.shadow.source\_radius} at distance $d$, so a physically larger or
nearer source gives a softer edge. Sampling the solid angle rather than a tangent-plane disk is what makes the real-time
penumbra agree with the path tracer's next-event estimation at any cone width, instead of only in the
small-angle limit where the two parameterisations coincide. \texttt{SHADOW_RAY_COUNT} is a
compile-time constant, a cost class rather than a live knob.

Directions come from spatiotemporal blue noise \cite{wolfe2022stbn}, two decorrelated dimensions per
light, spreading the two-sample penumbra across frames for the temporal accumulator to resolve
(Sections~\ref{sec:denoising} and \ref{sec:taa}). The sequence matters for two reasons a per-frame
image cannot show: a hash re-rolled every frame is white in time, leaving the accumulator nothing to
average down, and deriving both cone coordinates from one scalar collapses the cone to a curve. Per-light
dimensions also decorrelate the lights from each other, so their error falls with light count instead
of reinforcing. The cost is \(+0.088\) ms on the Forward pass, below the materiality floor of
...
angular size $\theta$, which is why Figure~\ref{fig:shadow-hs} frames the sunlit floor strip: the sun
is the only light here with a distant occluder, while the four enabled torches sit a couple of units
from the walls they light. Even there the true angular diameter gives about 36 pixels of a 2560-wide
capture, so the figure exaggerates it and says so.

Driving the sun rather than \texttt{render.shadow.source\_radius} is deliberate, for a reason that is
also a limitation. Cone samples falling below the shading normal's horizon are skipped while the
average still divides by the full sample count, which is correct in itself (counting them lit is the
leak next-event estimation rejects in the path tracer), although it means a wider cone darkens
grazing surfaces systematically. At a 1.5-unit radius that darkening is about three quarters of the
frame difference and swamps the penumbra it was meant to show. The near-vertical sun avoids it,
meeting the floor where nothing is horizon-rejected.
...
\texttt{render.shadows.rays} independent samples (this CVar drives only this path, not the inline one
above, whose ray count is the compile-time constant), and each sample separately uses resampled
importance sampling \cite{talbot2005ris} to pick \emph{one} light with
...
\caption[Reflections off versus ray-traced reflections]{Without reflections (left) versus with them
(right), cropped to the gold thread on a drape at a near-grazing angle where the Fresnel
contribution is largest. Cropped for the same reason as Figure~\ref{fig:ao-composited}: over the
whole frame the pass is worth 3.3 levels of 255, and here it is 11.8. What it adds is the thread
going from flat paint to something with a metallic sheen that varies as the surface turns. It is
worth saying what this figure cannot show, since it bounds the effect in this scene rather than in
the technique: Sponza has no mirror-like surface, so the reflection buffer returns some specular from
almost every material and a recognisable reflected image from none of them.}
