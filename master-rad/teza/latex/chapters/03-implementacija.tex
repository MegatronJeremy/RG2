...
\texttt{Platform/Vulkan/}; shaders are HLSL compiled to SPIR-V through dxc; the ImGui editor hosts the
CVar console and the performance readouts (Section~\ref{sec:alati}).
...
0.5 by default) and bilaterally upsampled before the forward pass samples them; reflections are
...
flagged \texttt{OPAQUE} so traversal skips the any-hit stage; alpha-cutout (glTF MASK) materials fall
back to a minimal any-hit alpha test unless an opacity micromap is available. The TLAS holds one
...
In the evaluation scenes the two-level structure buys nothing, since the model is authored as a
single unrepeated mesh set: \texttt{Sponza.world} holds 25 instances over 25 BLAS.
\texttt{Sponza-Motion.world} adds the four rotating props of Section~\ref{sec:motion}, four
transforms of one mesh, giving 29 instances over 26 BLAS. The instance count reported by
...
ones; without the extension every cutout microtriangle pays it. The bake runs once per mesh and is
...
evaluates a single bounce; shadows and AO need only a visibility answer and use the cheaper any-hit
traversal. Primary shading in the forward pass is a standard metallic-roughness Cook-Torrance model
...
ray straight at the light; the soft path casts \texttt{SHADOW_RAY_COUNT = 2} rays sampling, uniformly
...
light. Widening \texttt{render.shadow.source\_radius} instead would be the wrong control, for a reason
that is also a limitation. Cone samples falling below the shading normal's horizon are skipped while the
...
\texttt{render.ao.radius}; a hit contributes \texttt{1 - saturate(t / AORadius)}, so nearer occluders
...
by \texttt{ShadeSurfaceHit} (sun lighting with its own shadow ray, the hit surface's emissive term,
image-based ambient); a miss samples the prefiltered sky cube. Unlike the GI pass below, reflections
...
the Fresnel term, the BRDF weighting, and \texttt{render.reflections.intensity} are applied afterward
in the forward pass (Figure~\ref{fig:refl}).
