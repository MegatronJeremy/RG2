...
happen to sit between them. Grouping the compute passes that way lets the run pay one synchronisation
boundary instead of one per chain.
...
\caption[The motion vector debug view]{The motion vector buffer (\texttt{render.debugview} 1) during a deterministic camera orbit.
Red is the horizontal, green the vertical component of the pixel displacement between two
consecutive frames. The view takes the absolute value and multiplies it by 40, so it shows \emph{how
much} a pixel moves and along which axis, but not in which direction. Black is the absence of
displacement, and also the sky, which writes no vectors in this pass at all. The view is not
calibrated: the channels are normalised by width and height respectively, so their brightness is not
on the same scale, and the output is additionally sRGB encoded. Undoing that, the mean displacement
in this frame is about $0.6\%$ of the screen diagonal per frame, at most $1.6\%$, and no sample is
saturated.}
\label{fig:motion}
\end{figure}
